...
func (s *Server) VerifySunriseUser(c *gin.Context) {
	...
	if err = in.Validate(); err != nil {
		...
		c.HTML(http.StatusNotFound, "sunrise/status/404.html", scene.New(c))
		return
	}
	...
	if model, err = s.store.RetrieveSunrise(ctx, token.RecordID()); err != nil {
		...
		c.Error(err)
		c.HTML(http.StatusInternalServerError, "sunrise/status/500.html", scene.New(c))
		return
	}
...
func (s *Server) SunriseMessageAccept(c *gin.Context) {
	...
	// TODO: change errors to use front-end rather than JSON errors.
	in = &api.Envelope{}
	if err = c.BindJSON(&in); err != nil {
		c.Error(err)
		c.JSON(http.StatusBadRequest, api.Error("could not parse request"))
	...
	// TODO: redirect to completed page for both accept and reject
...
// TODO: REMOVE ME!
func (s *Server) SunriseDebug(c *gin.Context) {
	s.SunriseError(c, errors.New("whoopssie doopsie"))
}
