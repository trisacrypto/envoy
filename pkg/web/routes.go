...
func (s *Server) setupRoutes() (err error) {
	...
	ui := s.router.Group("", authenticate)
	{
		...
		ui.GET("/counterparties", authorize(permiss.CounterpartiesView), s.CounterpartiesListPage)
		ui.GET("/counterparties/:id", authorize(permiss.CounterpartiesView), s.CounterpartyDetailPage)
		ui.GET("/counterparties/:id/edit", authorize(permiss.CounterpartiesManage), s.CounterpartyEditPage)
	...
	v1 := s.router.Group("/v1")
	{
		...
		transactions := v1.Group("/transactions", authenticate)
		{
			...
			transactions.GET("/:id/latest", authorize(permiss.TravelRuleView), s.LatestEnvelope)
